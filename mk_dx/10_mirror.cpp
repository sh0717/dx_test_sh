...
mEyePosW(0.0f, 0.0f, 0.0f), mRenderOption(RenderOptions::Texture),
...
void TexturedHillsAndWavesApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		RenderStates::MarkMirrorOn(md3dImmediateContext);

		pass->Apply(0, md3dImmediateContext);
		md3dImmediateContext->Draw(6, 24);
		RenderStates::MarkMirrorOff(md3dImmediateContext);
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		RenderStates::DrawReflectOn(md3dImmediateContext);

		pass->Apply(0, md3dImmediateContext);
		md3dImmediateContext->DrawIndexed(mSkullIndexCount, 0, 0);

		RenderStates::DrawReflectOff(md3dImmediateContext);
...
void TexturedHillsAndWavesApp::BuildRoomGeometryBuffers()
{
